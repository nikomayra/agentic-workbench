...
Approved plan
    ↓
Create worktrees and work records
    ↓
Run work concurrently with temporary workers
    ├─ Any work paused → save every work record → await approvals
    │                       ↓
    │                  resume only paused work
    │
    └─ All work complete
             ↓
      Commit worker branches
             ↓
      Merge branches into integration worktree
             ├─ Conflict → repair conflict in integration worktree
             ↓
         Run tests
             ↓
          Review
             ├─ Changes required → repair → test → review
             ↓
       Await final approval / complete
             ↓
          Clean up
...
The key rule is:
    - A completed worker stays completed.
    - A pending worker retains its serialized RunState.
    - If any worker is pending, the coordinator is pending.
    - After decisions arrive, resume only pending workers.
    - Continue to integration only when every worker is completed.

Prepared WorkRecord:
- work_execution may be None

Returned WorkRecord:
- work_execution must exist
